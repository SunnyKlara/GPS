 ...
 *
 * 原始问题:
 *   - �?uart_dev_open() 只按"第一个空闲UART硬件"来选，不检查是否启�?
 *   - 结果选了被禁用的 UART0 �?CONFIG_UART0_ENABLE&&ut_num==0 不满�?
 *   - �?所有条件都失败 �?return NULL �?GPS UART 永远打不开
 *
 * 修复后逻辑:
 *   - CONFIG_UART0_ENABLE = 1: 保留（调试串口虽然TX/RX�?NO_CONFIG_PORT�?
 *     但硬件仍需开启，否则debug_uart_init会失败）
 *   - CONFIG_UART1_ENABLE = 1: 保留给GPS用（PB5�?
 *   - CONFIG_UART2_ENABLE = 0: 未使用，禁用
 *   - 真正的修复在 uart_dev_open()：改为同时检�?启用"�?空闲"
 *
 * 效果:
 *   调试串口: uart_dev_open(uart0_data) �?�?UART0（硬件空闲且启用）→ OK
 *   GPS串口:  uart_dev_open(gps_uart_arg) �?UART0已被占用，�?UART1 �?OK
 */
const u32 CONFIG_UART0_ENABLE = 1;   // 调试串口（TX/RX都用NO_CONFIG_PORT，但硬件要开启）

/* _WEAK_ */
/* extern */
const u32 CONFIG_UART1_ENABLE  = 1;  // GPS UART1（PB5 RX�?

/* _WEAK_ */
/* extern */
const u32 CONFIG_UART2_ENABLE  = 0;  // 未使用，禁用
 ...
 *
 * @param buf 字节存放地址
 * @param timeout 接收超时时间，单�?ms
 * @return 返回0：接收失败；返回1：接收成�?
 */
...
static void UT0_isr_fun(void)
{
    ...
    if ((JL_UART0->CON0 & BIT(3)) && (JL_UART0->CON0 & BIT(14))) {
        ...
        /* uart0.kfifo.buf_in += uart0.frame_length; //每满frame_length字节则产生一次中�?*/
    ...
    if ((JL_UART0->CON0 & BIT(5)) && (JL_UART0->CON0 & BIT(11))) {
        ...
        JL_UART0->CON0 |= BIT(12);                    //清RX PND(这里的顺序不能改变，这里要清一�?
        rx_len = JL_UART0->HRXCNT;             //读当前串口接收数据的个数
...
static u32 UT0_read_buf(u8 *buf, u32 len, u32 timeout)
{
    ...
    if (JL_UART0->CON0 & BIT(6)) {
        //DMA_MODE
        for (i = 0; i < len; i++) {
            if (!kfifo_get(&uart0.kfifo, buf + i, 1)) {
                UT_OSSemPend(&uart0.sem_rx, ut_msecs_to_jiffies(timeout));
                kfifo_get(&uart0.kfifo, buf + i, 1);
            }
        }
        return len;
    } else {
        _timeout = ut_msecs_to_jiffies(timeout) + ut_get_jiffies();
        _t_sleep = ut_msecs_to_jiffies(10) + ut_get_jiffies();
        for (i = 0; i < len; i++) {
            while (!(JL_UART0->CON0 & BIT(14))) {
                if (time_before(_timeout, ut_get_jiffies())) {
                    return i;
                }
...
static void UT1_isr_fun(void)
{
    ...
    if ((JL_UART1->CON0 & BIT(3)) && (JL_UART1->CON0 & BIT(14))) {
        ...
        /* uart1.kfifo.buf_in += uart1.frame_length; //每满32字节则产生一次中�?*/
    ...
    if ((JL_UART1->CON0 & BIT(5)) && (JL_UART1->CON0 & BIT(11))) {
        ...
        JL_UART1->CON0 |= BIT(12);                    //清RX PND(这里的顺序不能改变，这里要清一�?
        rx_len = JL_UART1->HRXCNT;             //读当前串口接收数据的个数
...
static u32 UT1_read_buf(u8 *buf, u32 len, u32 timeout)
{
    ...
    if (JL_UART1->CON0 & BIT(6)) {
        for (i = 0; i < len; i++) {
            if (!kfifo_get(&uart1.kfifo, buf + i, 1)) {
                UT_OSSemPend(&uart1.sem_rx, ut_msecs_to_jiffies(timeout));
                kfifo_get(&uart1.kfifo, buf + i, 1);
            }
        }
        return len;
    } else {
...
static void UT2_isr_fun(void)
{
    ...
    if ((JL_UART2->CON0 & BIT(3)) && (JL_UART2->CON0 & BIT(14))) {
        ...
        /* uart2.kfifo.buf_in += uart2.frame_length; //每满32字节则产生一次中�?*/
    ...
    if ((JL_UART2->CON0 & BIT(5)) && (JL_UART2->CON0 & BIT(11))) {
        ...
        JL_UART2->CON0 |= BIT(12);                    //清RX PND(这里的顺序不能改变，这里要清一�?
        rx_len = JL_UART2->HRXCNT;             //读当前串口接收数据的个数
...
static u32 UT2_read_buf(u8 *buf, u32 len, u32 timeout)
{
    ...
    if (JL_UART2->CON0 & BIT(6)) {
        for (i = 0; i < len; i++) {
            if (!kfifo_get(&uart2.kfifo, buf + i, 1)) {
                UT_OSSemPend(&uart2.sem_rx, ut_msecs_to_jiffies(timeout));
                kfifo_get(&uart2.kfifo, buf + i, 1);
            }
        }
        return len;
    } else {
...
const uart_bus_t *uart_dev_open(const struct uart_platform_data_t *arg)
{
    ...
    /*
     * 修复: 同时检�?UART 是否空闲 AND 是否启用
     * 原代码只检查是否空闲，导致选择了被禁用�?UART0 后条件不匹配返回 NULL
     */
